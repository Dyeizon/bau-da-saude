...
import { CSS, fetchUrl, formatDatePT, getTokenID } from "../utils";
...
export const Charts: React.FC = () => {
    ...
    const primaryAxis = useMemo(
      ...
      const secondaryAxes = useMemo(
        (): AxisOptions<ChartDatum>[] => [
          {
            getValue: datum => datum.resultValue,
            min: 0,
            elementType: 'area'
          },
        ],
    ...
    const fetchResultTypes = async () => {
      ...
      const generateChart = async () => {
        ...
        let url = `${fetchUrl}/exams/data/select?owner=${getTokenID(localStorage.getItem('token')?.split(' ')[1])}&examType=${examType}&resultType=${resultType}`;
        
        if(selectFilterType === "result-and-date") url += `&startDate=${startDate}&endDate=${endDate}`;
        ...
        try {
            ...
            const fixedData = response.data.results.map((r: any) => ({
                resultValue: parseFloat(r.resultValue),
                date: formatDatePT(new Date(r.date)),
            }));
            ...
            // console.error("Erro ao consultar os dados para o gráfico:", error);
            data = [
                    {
                        label: "",
                        data: [
        ...
        <>
            ...
            <div className="w-full mb-5 group">
                ...
                <select id="filterType" onChange={e => setSelectFilterType(e.currentTarget.value)} value={selectFilterType} className="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                    <option value="result">Tipo de resultado</option>
                ...
                <div className="grid md:grid-cols-2 md:gap-5">
                    <div>
                        ...
                        >
                            ...
                            {examTypes.map((type) => {
                                if (type.name !== "Exame de Imagem") return (
